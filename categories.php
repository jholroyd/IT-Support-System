...
if(isset($_POST['edit_category'])) {
    ...
    edit_category($id, $edit_category_name);
...
if(isset($_POST['import_csv'])) {
    if($_FILES['csv']['size'] > 0) {
        ...
        import_categories_csv($file, $handle);
    } else {
...
<head>
    ...
    <!-- title -->
    <title><?php if($_SESSION['role'] == 2){ ?>Access Denied<?php } else { ?>Categories<?php } ?> - Support</title>
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Category: #<?php echo $result['0']; ?></h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <div class="row">
                    ...
                    <div class="well">
                        <form role="form" method="post" action="" class="no-margin">
                            ...
                            <div class="form-group" style="margin-bottom:30px;">
                                <label>Name</label>
                                <input type="text" class="form-control" name="edit_category_name" value="<?php echo $result['name']; ?>" />
                            </div>
                            <div class="form-group" style="margin-bottom:0;">
                                <a href="categories.php" class="btn btn-default"><i class="fa fa-arrow-left"></i> Back</a>
                                <button type="submit" class="btn btn-success pull-right" name="edit_category">Save Changes</button>
                            </div>
                        </form>
                    </div>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Device Doesn't Exist</h1>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                    <p>Sorry, but the category you are trying to access doesn't exist. Use the button below to go back to the categories page.</p>
                    <a href="categories.php" class="btn btn-success"><i class="fa fa-arrow-left"></i> Go to Categories</a>
                </div>
            </div>
            <?php } ?>
            <?php } else { ?>
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-heading">
                            ...
                            <div class="pull-right">
                                <div class="btn-group">
                                    <button type="button" data-toggle="modal" data-target="#myModal" class="btn btn-default btn-xs">Create New Category</button>
									<!-- Modal -->
											...
											<div class="modal-content">
												...
												<div class="modal-body">
													<form role="form" action="" method="post">
														<div class="form-group">
                                                            <input type="text" class="form-control" name="category_name" placeholder="Enter a category here ..." />
														</div>
												</div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                                                    <button type="submit" class="btn btn-success" name="new_category">Create Category</button>
                                                    </form>
                                                </div>
											</div>
											<!-- /.modal-content -->
                        ...
                        <div class="panel-body">
                                ...
                                <table class="table no-margin">
                                    ...
                                    <tbody>
                                        ...
                                        foreach($query as $row){
                                            echo '<tr>';
                                                echo '<td>'.$row['name'].'</td>';
                                                echo "<td><a href='?edit=".$row['id']."' class='btn btn-default btn-xs'><i class='fa fa-edit'></i> Edit</a> <form method='post' action='' class='form-inline'><button type='submit' name='delete_category' class='btn btn-danger btn-xs' onclick=\"return confirm('Are you sure you want to delete this category?')\"><i class='fa fa-remove'></i> Delete</button><input type='hidden' name='del_category_id' value='".$row['id']."' /></form></td>";
                                            echo '</tr>';
                                        }
                                        ?>
                                    </tbody>
                            ...
                            <?php } else { ?>
                            <p>There have been no categories created yet.</p>
                            <?php } ?>
                            <!-- /.table-responsive -->
                        </div>
